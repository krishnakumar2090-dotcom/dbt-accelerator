...
irt_enrollment_aggregated AS (
    SELECT
        trial_id,
        site_id,
        SUM(subjects_enrolled) AS subject_count, -- Aggregated at trial, site level
        planned_subjects AS enrollment_target -- Direct mapping
...
FROM vcv_trials_clean vt
LEFT JOIN vcv_sites_clean vs
